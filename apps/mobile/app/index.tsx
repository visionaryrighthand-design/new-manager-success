...
import { module01, estimateSectionTotalSeconds } from '@nms/content';
import { moduleCompletionPercent, nextSectionFor } from '@nms/core';
...
export default function HomeScreen() {
  ...
  const percent = moduleCompletionPercent(progress, sectionIds);
  const upNextId = nextSectionFor(progress) ?? module01.sections[0]!.id;
  const upNext = module01.sections.find((r) => r.id === upNextId)!;
  ...
  return (
    ...
    >
      ...
      {progress.streak.atRisk ? (
        <View style={styles.riskBanner}>
          <Text style={styles.riskText}>
            Your {progress.streak.current}-day streak runs out tonight. One Section keeps it.
          </Text>
        </View>
      ) : null}

      {/* Up next — the single most important control on this screen. */}
      <Link href={`/section/${upNext.id}`} asChild>
        <Pressable style={styles.upNext}>
          <Text style={styles.upNextLabel}>
            {percent === 0 ? 'START HERE' : percent === 100 ? 'RUN IT BACK' : 'UP NEXT'}
          </Text>
          <Text style={styles.upNextNumber}>Section {upNext.number}</Text>
          ...
          <View style={styles.upNextMeta}>
            <Text style={styles.upNextMetaText}>
              {Math.round(estimateSectionTotalSeconds(upNext) / 60)} min
            </Text>
            <Text style={styles.upNextMetaText}>
      ...
      {module01.sections.map((section) => {
        const done = Boolean(progress.sections[section.id]?.completedAt);
        const score = progress.sections[section.id]?.bestScore;
        return (
          <Link key={section.id} href={`/section/${section.id}`} asChild>
            <Pressable style={[styles.repRow, done && styles.repRowDone]}>
              <View style={[styles.repDot, done && styles.repDotDone]}>
                <Text style={[styles.repDotText, done && styles.repDotTextDone]}>
                  {done ? '✓' : section.number.split('.')[1]}
                </Text>
              </View>
              <View style={styles.sectionBody}>
                <Text style={styles.sectionTitle}>{section.title}</Text>
                <Text style={styles.sectionHook} numberOfLines={2}>
                  {section.hook}
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { ...type.bodyStrong, color: colors.fg },
  sectionHook: { ...type.caption, color: colors.fgMuted },

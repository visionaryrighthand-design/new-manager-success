 ...
 * prints per-Section timing against the script's own targets.
 *
 ...
 *   TOTAL — what the Section actually costs a learner in the app, including
...
for (const mod of course.modules) {
  ...
  console.log('-'.repeat(78));
  console.log('Section    Title                                  Target   NARR   TOTAL  Beats  Q  CB');
  ...
  for (const section of mod.sections) {
    const narration = estimateSectionSeconds(section);
    const inApp = estimateSectionTotalSeconds(section);
    totalTarget += section.durationMinutes * 60;
    totalNarration += narration;
    totalInApp += inApp;

    console.log(
      [
        section.number.padEnd(6),
        section.title.slice(0, 38).padEnd(38),
        `${section.durationMinutes}m`.padStart(6),
        fmt(narration).padStart(6),
        fmt(inApp).padStart(7),
        String(section.beats.length).padStart(6),
        String(section.quiz.length).padStart(3),
        section.videoUrl ? ' ✓ ' : '  ·',
      ].join(' '),
    );
  }
  ...
  console.log(
    `         Sections ${mod.sections.length} · questions ${mod.sections.reduce((n, r) => n + r.quiz.length, 0)}` +
      ` · films ${mod.sections.filter((r) => r.videoUrl).length}/${mod.sections.length}`,
  );
  ...
  if (shortfall > 300) {
    console.log(
      `\n  NOTE: narration runs ${fmt(shortfall)} under the script headers' combined target.\n` +
        ...
        `  In-app time lands at ${fmt(Math.round(totalInApp / mod.sections.length))} per Section once interactions\n` +
        `  are counted. Decision needed: accept the shorter Section, or commission more script.\n` +

...
export function allowedContactRoles(entry: RegistrationEntry): readonly ContactRole[] {
  return entry === 'self'
    ? (['boss', 'hr', 'other'] as const)
    : (['boss', 'hr', 'other', 'registrant'] as const);
}
...
export const selfContactRoleLabels: Record<Exclude<ContactRole, 'registrant'>, string> = {
  boss: 'My boss',
  hr: 'HR',
  other: 'Someone else',
};

export const updateLevelSummary: Record<UpdateLevel, { name: string; cadence: string; contents: string }> = {
  1: {
    name: 'Basic',
  ...
  2: {
    ...
    contents:
      'Sections completed that week, test and quiz scores, and a running total of days since they started.',
  },
  3: {
    name: 'Advanced',
    cadence: 'Weekly digest',
    contents:
      'Everything in Level 2, plus suggested questions to ask them in your next one-to-one.',
  },
};
 ...
 */
export function validateEnrollment(enrollment: Enrollment): ValidationProblem[] {
  const problems: ValidationProblem[] = [];
  const { entry, student, contacts, registeredBy } = enrollment;

  if (!student.name.trim()) problems.push({ field: 'student.name', message: 'Student name is required.' });
  ...
  const seenEmails = new Set<string>([student.email.trim().toLowerCase()]);

  contacts.forEach((contact, i) => {
    const at = `contacts[${i}]`;
    if (!contact.name.trim()) problems.push({ field: `${at}.name`, message: 'Contact name is required.' });
    ...
    if (!allowed.includes(contact.role)) {
      problems.push({
        field: `${at}.role`,
        message: `Role "${contact.role}" is not offered when ${
          entry === 'self' ? 'registering yourself' : 'registering someone else'
        }.`,
      });
    }
 ...
 */
export function categorize(enrollment: Pick<Enrollment, 'entry' | 'contacts'>): EnrollmentCategory {
  if (enrollment.entry === 'other-group') return 'compliance-bulk';
  if (enrollment.entry === 'other-individual') return 'manager-visibility';
  return enrollment.contacts.length === 0 ? 'self-directed' : 'manager-visibility';
}

export const categoryLabels: Record<EnrollmentCategory, { title: string; description: string }> = {
  'self-directed': {
    title: 'Self-directed',
  ...
  'compliance-bulk': {
    ...
    description:
      'Typically HR or L&D enrolling several people at once. Each student can still have their own contacts and levels — for example, each new manager’s own supervisor.',
  },
};

/** Contacts on this enrollment at a given level. */
export function contactsAtLevel(enrollment: Enrollment, level: UpdateLevel): Contact[] {
 ...
 */
export function contactsAtOrAbove(enrollment: Enrollment, level: UpdateLevel): Contact[] {
  return enrollment.contacts.filter((c) => c.level >= level);
}

...
export interface BusinessDayOptions {
  /** IANA zone the tenant operates in. Defaults to UTC. */
  timeZone?: string;
  /** Dates to treat as non-business days, as 'YYYY-MM-DD'. */
  holidays?: readonly CalendarDate[];
  /**
   * Days of the week that count as business days, 0=Sunday..6=Saturday.
   * Defaults to Mon–Fri. Exposed because not every pilot client works
   * Mon–Fri — restaurants and clinics are explicitly in the target market.
   */
  businessWeekdays?: readonly number[];
}

const DEFAULT_BUSINESS_WEEKDAYS = [1, 2, 3, 4, 5] as const;
const MS_PER_DAY = 86_400_000;

/** Formats an instant as a calendar date in the given zone. */
export function toCalendarDate(instant: Date, timeZone = 'UTC'): CalendarDate {
...
export function fromCalendarDate(date: CalendarDate): Date {
  ...
  if (!parsed) throw new RangeError(`Not a calendar date: ${date}`);
  const [, y, m, d] = parsed;
...
export function isBusinessDay(date: CalendarDate, options: BusinessDayOptions = {}): boolean {
  ...
  if (!weekdays.includes(dayOfWeek(date))) return false;
...
export function businessDaysBetween(
  ...
  while (cursor < to) {
    cursor = addCalendarDays(cursor, 1);
    if (isBusinessDay(cursor, options)) count += 1;
  }
  return count;
}

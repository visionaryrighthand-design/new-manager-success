import { addBusinessDays, businessDaysBetween, toCalendarDate, } from '../time/business-days.js';
/**
...
export function evaluateInactivity(input) {
    ...
    const nextAlertDate = addBusinessDays(lastActivity, (alertsDueSoFar + 1) * INACTIVITY_THRESHOLD_BUSINESS_DAYS, opts);
    if (alertsDueSoFar === 0) {
    ...
    const alertsAlreadySent = input.lastAlertSentAt
        ? // Alerts sent before the last activity belong to a previous dry spell.
            toCalendarDate(input.lastAlertSentAt, tz) >= lastActivity
                ? Math.floor(businessDaysBetween(lastActivity, toCalendarDate(input.lastAlertSentAt, tz), opts) /

 ...
 */
export function sectionCompleteNotifications(enrollment, repId) {
 ...
 */
export function weeklyDigestNotifications(input) {
    ...
    const scoredThisWeek = Object.values(progress.reps).flatMap((r) => r.attempts
    ...
    return recipients.map((contact) => {
        ...
        return {
            ...
            subject: `${name}’s week: ${completedThisWeek.length} section${completedThisWeek.length === 1 ? '' : 's'} completed`,
            body,
        };
    });
}
 ...
 */
export function inactivityNotifications(input) {
    ...
    const notifications = input.enrollment.contacts.map((contact) => ({
        kind: 'inactivity-alert',
        to: contact,
